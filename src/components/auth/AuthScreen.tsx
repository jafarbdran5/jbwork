  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err?.message === 'UNAUTHORIZED_ACCOUNT') {
        ...
        setErrorMsg(
          isRTL 
            ? 'فشل تسجيل الدخول بحساب Google. يرجى المحاولة مرة أخرى أو استخدام البريد المعتمد.' 
            : 'Google Sign-in failed. Please try again or use authorized email.'
        );
      }
    } finally {
      setLoading(false);
          ...
          {activeMode === 'google' && (
            <div className="space-y-4">
              ...
              >
                ...
                <span>{loading ? (isRTL ? 'جارٍ الاتصال بـ Google...' : 'Connecting to Google...') : (isRTL ? 'تسجيل الدخول المباشر بحساب Google' : 'Sign in with Google Account')}</span>
              </button>
